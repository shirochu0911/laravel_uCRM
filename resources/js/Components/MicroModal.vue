...
<template>
        ...
        <div class="modal__overlay" tabindex="-1" data-micromodal-close>
            <div class="modal__container w-2/3" role="dialog" aria-modal="true" aria-labelledby="modal-1-title">
                <header class="modal__header">
                    ...
                    <button type="button" @click.prevent="toggleStatus" class="modal__close" aria-label="Close modal"
                        ...
                        <table class="w-full text-left whitespace-no-wrap table-auto">
                            ...
                            <tbody>
                                <tr v-for="customer in customers.value.data" :key="customer.id">
                                    <td class="px-4 py-3 border-b-2 border-gray-200"><button type="button"
                                            @click.prevent="setCustomer({ id: customer.id, kana: customer.kana })"
                                            class="text-blue-400">{{ customer.id }}</button></td>
                ...
                <footer class="modal__footer">
                    <button type="button" @click.prevent="toggleStatus" class="modal__btn" data-micromodal-close
    ...
    <button type="button" @click.prevent="searchCustomers" data-micromodal-trigger="modal-1"
        class="flex px-8 py-2 mx-auto text-lg text-white bg-teal-500 border-0 rounded focus:outline-none hover:bg-teal-600">検索する</button>
</template>